  ...
  getMemberTaskStats(member: string): GangTask[] {
    ...
    for (const task of this.gangTasks) {
      ...
      const tempTask: GangTask = {
        ...
        agiExp: exp?.agi_exp ?? 0,
        chaExp: exp?.cha_exp ?? 0,
        defExp: exp?.def_exp ?? 0,
        dexExp: exp?.dex_exp ?? 0,
        strExp: exp?.str_exp ?? 0,
        hackExp: exp?.hack_exp ?? 0
      };
...
export async function main(ns: NS) {
  ns.disableLog("ALL");
  const gangEngine = new GangEngine(ns, ns.args);
  await gangEngine.start();
}
